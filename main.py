...
async def main(audio_file_path: str):
    ...
    whisper_model = config.get("whisper", {}).get("model_name", "base")
    ...
    transcriber = Transcriber(model_name=whisper_model, device=device, compute_type=compute_type)
    aligner = ForcedAligner(device=device)
    llm_handler = LLMOrchestrator(config_path=config_path, prompt_config_path=prompt_path, model_id="llama")
    ...
    # Free enhancement model before loading next
    del enhancer
    gc.collect()
    ...
    #  Step 14: File/Audio Feature Extraction
    ...
    # Step 16.2: Insert Utterance Table
    utterances = final_output["ssm"]

    for utterance in utterances:
        file_id = last_id
        speaker = utterance.get("speaker", "Customer")
        sequence = utterance.get("index", 0)
        ...
        profane = 1 if utterance.get("profane", False) else 0
