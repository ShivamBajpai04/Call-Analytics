...
class LLaMAModel(LanguageModel):
    ...
    def generate(
        ...
        str
            Generated text.
        """
        prompt = self._format_messages(self.tokenizer, messages)
        output = self.pipe(
            prompt,
    ...
    @staticmethod
    def _format_messages(tokenizer, messages: Annotated[list, "List of message dictionaries"]) -> Annotated[
        str, "Formatted prompt"]:
        """
        Format messages using the tokenizer's chat template if available,
        otherwise fall back to a generic format.

        Parameters
        ----------
        tokenizer
            The model's tokenizer (used for apply_chat_template).
        messages : list
            List of message dictionaries with 'role' and 'content'.
...
class LanguageModelManager:
    ...
    def __init__(
            ...
            cache_size: Annotated[int, "Maximum number of models to cache"] = 1
    ):
